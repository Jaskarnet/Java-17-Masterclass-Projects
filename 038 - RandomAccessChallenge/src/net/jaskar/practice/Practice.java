...
public class Practice {
    ...
    static {
        try {
            RandomAccessFile rb = new RandomAccessFile("resources/student.idx", "r");
            loadIndex(rb, 0);
        } catch (IOException e) {
...
//        BuildStudentData.build("student", true);
        try (RandomAccessFile ra = new RandomAccessFile("resources/student.dat", "r")) {
            ...
            while (scanner.hasNext()) {
                ...
                ra.seek(indexesIds.get(studentId));
    ...
    private static void loadIndex(RandomAccessFile ra, int indexPosition) {
        try {
            ...
            for (int i = 0; i < recordsInFile; i++) {
                indexesIds.put(ra.readLong(), ra.readLong());
            }
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
